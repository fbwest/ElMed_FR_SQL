...
select
	count(distinct t.NPOLIS) as s2001_1,
	count(distinct case when DS1_PR = 1 then t.NPOLIS end) as s2001_2,
	count(distinct case when dnMax <= 2 then t.NPOLIS end) as s2001_3,
	count(distinct case when dnMax <= 2	and (b18 > 0 and k74 > 0) then t.NPOLIS end) as s2003_1,
	count(distinct case when dnMax <= 2	and (b18 > 0 and c22 > 0) then t.NPOLIS end) as s2003_2,
	count(distinct case when DN = 2 and rowNum = '10.0' then t.NPOLIS end) as s2004_1,
	count(distinct case when DN = 2 and rowNum = '10.0' and dnMax >= 4 then t.NPOLIS end) as s2004_2
from @form m
	left join @tempTable t on
		t.DS1 between m.dsFrom and m.dsTo or
		t.DS1 like m.dsLike or
		t.DS1 like m.dsLike2
	left join (
		select NPOLIS,
