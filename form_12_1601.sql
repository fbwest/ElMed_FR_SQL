...
select
	count(distinct t.NPOLIS) as s1601_1,
	count(distinct case when floor(datediff(day, DR, DATE_1) / 365.25) < 1 then t.NPOLIS end) as s1601_2,
	count(distinct case when DS1_PR = 1 then t.NPOLIS end) as s1601_3,
	count(distinct case when DS1_PR = 1
		and floor(datediff(day, DR, DATE_1) / 365.25) < 1 then t.NPOLIS end) as s1601_4,
	count(distinct case when dnMax <= 2	then t.NPOLIS end) as s1601_5,
	count(distinct case when dnMax <= 2
		and floor(datediff(day, DR, DATE_1) / 365.25) < 1 then t.NPOLIS end) as s1601_6
from @form m
	left join @tempTable t on
		t.DS1 between m.dsFrom and m.dsTo
		or t.DS1 like m.dsLike
	left join (
		select NPOLIS, max(DN) as dnMax
		from @tempTable
		group by NPOLIS
