...
insert into @formT values
	('01', 'A50-A53', 'A5[0-3]%', N'Сифилис – все формы'),
...
declare @result table (dsName nvarchar(100), rowNum nvarchar(5), c3 int)
insert into @result
    select dsName, rowNum, count(distinct ID) as c3
    from @formT	f left join @sampleT on DS1 like dsLike
    group by dsName, rowNum

insert into @result
    select N'Всего', '07', sum(c3)
    from @result
